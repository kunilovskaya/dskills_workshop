...
\begin{description}
	...
	\item[tabularx] uses X columns to f%\usepackage{pgfplots}
	%\usepackage{pgfplotstable}it the table to the specified width (\verb-{\begin{tabular}{\textwidth}{X|c|X}-)
...
\begin{figure}[h] % htbp
		...
		\setlength\fboxsep{5pt} % set its distance of the frame \fbox{} from the content
